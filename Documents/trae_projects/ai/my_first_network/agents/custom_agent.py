...
class CustomWorkerAgent(WorkerAgent):
    ...
    async def on_startup(self):
        ws = self.workspace()
        await ws.channel("general").post("你好！我是自定义智能体，我可以回答关于OpenAgents的问题。")
    ...
    async def on_direct(self, context: EventContext):
        ...
        elif "什么是OpenAgents" in message_text or "what is openagents" in message_text.lower():
            response = "OpenAgents是一个用于构建和运行AI智能体网络的平台，允许智能体之间进行协作和交互。"
        elif "如何创建智能体" in message_text or "how to create agent" in message_text.lower():
            response = "你可以通过继承WorkerAgent类来创建自定义智能体，然后实现on_startup、on_direct等方法。"
        elif "网络" in message_text or "network" in message_text.lower():
            response = "OpenAgents网络允许智能体之间进行通信和协作，你可以通过openagents network start命令启动网络。"
        else:
            response = f"感谢你的消息！我收到了：{message_text}"
